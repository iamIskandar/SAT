<?php
  include ('sessionadmin.php');
  include 'header.php';
  include ('dbconnect.php');

  if(isset($_GET['no']))
  {
    $cno = $_GET['no'];
  }

  $sql = "SELECT * FROM tb_chicken
          LEFT JOIN tb_project ON tb_chicken.c_id = tb_project.p_id
          LEFT JOIN tb_status ON tb_chicken.c_status = tb_status.s_id
          LEFT JOIN tb_temporary ON tb_chicken.c_no = tb_temporary.t_no
          WHERE c_no='$cno'";
  $result = mysqli_query($con, $sql);
  $row = mysqli_fetch_array($result);
?>

  <div class="content-wrapper">
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>Modification Request Approval</h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <?php
                echo"<h3>Today: ".date("d-m-y")."</h3>";
              ?>
            </ol>
          </div>
        </div>
      </div>
    </section>

    <section class="content">
      <div class="row">
        <div class="col-md-6">
          <div class="card card-warning">
            <div class="card-header">
              <h3 class="card-title">Overview</h3>
            </div>
            <form>
              <div class="card-body">
                <h4>Current Record Details</h4>
                <table class="table table-hover">
                  <tbody>
                    <tr><td><b>Project Name: </b><?php echo $row['p_name'];?></td></tr>
                    <tr><td><b>Date: </b><?php echo $row['c_dateRec'];?></td></tr>
                    <?php
                      $sql1 = "SELECT * FROM tb_status WHERE s_id='".$row['c_feedType']."'";
                      $result1 = mysqli_query($con,$sql1);
                      $row1 = mysqli_fetch_array($result1);
                    ?>
                    <tr><td><b>Feed Type: </b><?php echo $row1['s_desc'];?></td></tr>
                    <tr><td><b>Feed Quantity: </b><?php echo $row['c_feedQty'];?></td></tr>
                    <tr><td><b>Death Quantity: </b><?php echo $row['c_deathQty'];?></td></tr>
                    <tr><td><b>Culls Quantity: </b><?php echo $row['c_cullsQty'];?></td></tr>
                    <tr><td><b>Remarks: </b>
                      <?php 
                      if(!$row['c_desc'])
                      {
                        echo "<b>-</b>";
                      }
                      else
                      {
                        echo $row['c_desc'];  
                      }
                      ?>
                    </td></tr>
                    <?php
                      if($row['c_avgWeight'])
                      {
                        echo '<tr><td><b>Average Weight (Kg): </b>'.$row['c_avgWeight'].'</td></tr>';
                      }
                    ?>
                  </tbody>
                </table>
              </div>
            </form>
          </div>
        </div>  
        <div class="col-md-6">
          <div class="card card-warning">
            <div class="card-header">
              <h3 class="card-title">Overview</h3>
            </div>
            <div class="card-body">
              <h4>New Record Details</h4>
              <table class="table table-hover">
                <tbody>
                  <tr><td><b>Project Name: </b><?php echo $row['p_name'];?></td></tr>
                  <tr><td><b>Date: </b><?php echo $row['t_dateRec'];?></td></tr>
                  <?php
                    $sql2 = "SELECT * FROM tb_status WHERE s_id='".$row['t_feedType']."'";
                    $result2 = mysqli_query($con,$sql2);
                    $row2 = mysqli_fetch_array($result2);
                  ?>
                  <tr><td><b>Feed Type: </b><?php echo $row2['s_desc'];?></td></tr>
                  <tr><td><b>Feed Quantity: </b><?php echo $row['t_feedQty'];?></td></tr>
                  <tr><td><b>Death Quantity: </b><?php echo $row['t_deathQty'];?></td></tr>
                  <tr><td><b>Culls Quantity: </b><?php echo $row['t_cullsQty'];?></td></tr>
                  <tr><td><b>Remarks: </b><?php echo $row['t_desc'];?></td></tr>
                  <?php
                    if($row['t_avgWeight'])
                    {
                      echo '<tr><td><b>Average Weight (Kg): </b>'.$row['t_avgWeight'].'</td></tr>';
                    }
                  ?>
                </tbody>
              </table>
              <br>
              <a href="approvemodify.php?no=<?php echo $row['c_no'];?>" class="btn btn-success">Approve</a>&nbsp&nbsp
              <a href="declinemodify.php?no=<?php echo $row['c_no'];?>" class="btn btn-danger">Decline</a>
              <a href="modifyapprovallist.php" class="btn btn-secondary float-right">Cancel</a>
            </div>
          </div>
        </div>
      </div>
    </section>
  </div>

<?php include 'footer.php'; ?>